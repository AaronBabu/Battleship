...
(*[print_column_label array] takes in an array and prints the labels of column of the
grid*)
...
(*[print_grid_ai array row_idx] takes in an array and int row_idx which represents
the index of a row and prints the entire grid with row and column labels*)
...
| Down
val index : char -> int
(** [index c ] accesses the index or position of a char c and returns the integer
 representation of the char*)


val inverse_index :  int -> char
(* [inverse_index i] acesses the inverse index i and outputs a char based on the
 index*)
val create_endcoords_shiplength2 : direction ->  char *int -> char* int
(*[create_endcoords_shiplength2 direction (char,int)] takes in a valid direction
(Up,Down,Left,Right) and a pair of valid coordinates (char,int) and outputs
the endcoordinates of a ship of length two that starts at the coordinate and
extends in the input direction. *)
val create_endcoords_shiplength3 : direction ->  char *int -> char* int
(*[create_endcoords_shiplength3 direction (char,int)] takes in a valid direction
(Up,Down,Left,Right) and a pair of valid coordinates (char,int) and outputs
the endcoordinates of a ship of length three that starts at the coordinate and
extends in the input direction. *)
val create_endcoords_shiplength4 : direction ->  char *int -> char* int
(*[create_endcoords_shiplength4 direction (char,int)] takes in a valid direction
(Up,Down,Left,Right) and a pair of valid coordinates (char,int) and outputs
the endcoordinates of a ship of length four that starts at the coordinate and
extends in the input direction. *)
val create_endcoords_shiplength5 : direction ->  char *int -> char* int
(*[create_endcoords_shiplength5 direction (char,int)] takes in a valid direction
(Up,Down,Left,Right) and a pair of valid coordinates (char,int) and outputs
the endcoordinates of a ship of length four that starts at the coordinate and
extends in the input direction. *)
...
(* [print_ship input_grid start_row start_col end_row end_col] takes in input
 ...
 ship on the grid as valid coordinates, and prints the updated input_grid with
 the ship location marked with # *)
