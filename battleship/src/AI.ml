...
let neighboring_points board =
  let neighbors (col, row) =
    ...
    List.filter
      (fun (c, r) ->
        r >= 0 && r <= 9 && c >= 65 && c <= 74 &&
        (List.nth board r).(c - 65) <> "o" && (List.nth board r).(c - 65) <> "x")
      (List.map (fun (dc, dr) -> (char_code + dc, row + dr)) directions)
  in
  List.fold_left (fun acc row_arr ->
    List.fold_left (fun inner_acc cell ->
      let col, row = cell in
  ...
  ) [] (List.mapi (fun i row -> Array.mapi (fun j cell -> (Char.chr (j + 65), i)) row) board)
...
let rec pick_random_point board =
  ...
  let col_code, row =
    if List.length available_neighbors > 0 then
      List.nth available_neighbors (Random.int (List.length available_neighbors))
    else
  ...
  | "#" ->
      ...
      (Char.chr col_code, row + 1, "x")  (* Return the updated coordinate and the symbol placed *)
  | _ ->
      (List.nth board row).(col_code - 65) <- "o";
      (Char.chr col_code, row + 1, "o")  (* Return the updated coordinate and the symbol placed *)