...
(**[letter_randomizer ()] takes in unit and randomizes the letters for the ai
   grid*)

val number_randomizer : unit -> int
(**[number_randomizer ()] takes in unit and randomizes the numbers for the ai
   grid*)
...
val generate_ship_coords :
  int -> t * int -> t -> (t * int) list -> (t * int) list
(** [generate_ship_coords len coord dir acc] takes in len which represents the
    ...
    the current list of ship coordinates being generated which all outputs the
    final list of ship coordinates generated. *)
...
(** [place_ships grid ships ship_coords] takes in a grid which represents the
    battleship grid, a ship to be placed on the grid, and ship_coords which is a
    list of coordinates that have already been place on the grid which outputs a
    list of coordinates for the new ship being placed. *)

val ywingCoords : (t * int) list
(** [ywingCoords] places ywing on the random grid by calling place_ship passing
    ywing and [] to represent the occupied coordinates from before which all
    returns the current coordinates of the ship *)

val xwingCoords : (t * int) list
(** [xwingCoords] places xwing on the random grid by calling place_ship passing
    xwing and ywing to represent the occupied coordinates from before which all
    returns the current coordinates of the ship*)
...
(**[place_ship_on_grid grid ship_coords] which takes in a grid and a list of the
   pairs of ship coordinates which all updates the input grid with the
   coordinates of the ship which is represented by the ship_coords list*)
