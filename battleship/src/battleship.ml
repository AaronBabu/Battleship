...
let print_rowlines n=
print_string ("  "^(get_rowlines (n) "") ^ "+")

let rec print_row row: unit = (*print_string "|__|";*) 
Array.iter (fun x -> print_string ("| " ^ x ^ " " ) ) row; print_string "|"
...
let make_play= print_column_label grid; print_grid grid 1